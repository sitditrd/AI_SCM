...
**화면별**: Port Insight(PCI 혼잡지수·포트검색), 선석배정(16터미널 고급 그리드·항만 기상·**조회결과 우클릭 Excel 내보내기**), 선박위치(AIS·**PORT-MIS 입출항 실적 조회**·**자체 AIS 수신 지도**(Leaflet, 5분 재조회)), 화물추적(**선사 API 실조회 6사** ONE·COSCO·SM상선·Evergreen·SITC·ZIM · **계정별 자동감시·변경 메일 알림** · 국내 터미널 교차검증 · 그 외 선사·항공사 딥링크), 경로분석(몬테카를로·Voyager 지도 항로 시각화), 해외 스케줄(선박 3뷰 준비중 · **항공 화물편 실조회**), 데이터현황(운영보드·**관리자 전용**). **국내/해외 탭은 선석배정↔해외스케줄에만** 노출. 미로그인 사용자는 처음엔 정상 노출 → 일정시간 뒤 주요기능 blur + 로그인 유도. **홈페이지는 한/영/중 다국어 전환**(우상단 언어 스위처, `js/i18n.js`).
...
**환경변수**: `SUPABASE_SERVICE_KEY` **등록 완료(2026-08-03, 사용자 환경변수)** — ② REST 직접 적재 가동을 위해 **②만 예외적으로 PC에 보관**(③④⑤⑥⑦은 종전대로 `--sql` 생성 후 Supabase MCP로 적재) · `AISSTREAM_API_KEY` **등록 완료** → ⑤ 가동 · `DATA_GO_KR_KEY`(Edge Function `datago`) **등록 완료(2026-08-03)** — 활용신청 12종 승인 + 별칭 15종 실조회 검증 · `UNIPASS_API_KEY`(Edge Function `track`)는 **미등록 대기** — 등록 즉시 코드 수정 없이 동작.
...
5. ~~**화물 추적 실조회**~~ — **완료(2026-08-19)**: 선사 트래킹 API 직접 조회 6사 가동(`carrier-track`) + 감시·메일 알림(`bl-watch`·`notify-bl`) 실운영
...
7. **선사 확장 3사** — 머스크(미신청·고객코드 필요) · HMM·하파그(승인 대기). 어댑터는 3사 모두 완성되어 **키 등록 즉시 라이브 승격**. 커버리지 산출·영업 확보 항목은 `docs/05-development/선사API_확장플랜_2026-08-19.docx`
