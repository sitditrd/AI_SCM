...
현황: 정적 웹 8화면 + Supabase 3계층. 기존 데이터 — 선석배정 16터미널(자체 수집), IMF PortWatch 기반 PCI, SCFI/CCFI, Open-Meteo 기상(예보), VesselFinder AIS(iframe), UNIPASS 프록시(키 미등록), 경로분석 93개 사전계산 항로, 해외 스케줄 준비중(FR-04/05).
...
| 자체 실시간 위치 지도(§5 참조) | AISStream.io + PORT-MIS | 중 | P2 |
...
| 후보 기능 | 필요 API | 난이도 | 우선순위 |
|---|---|---|---|
| 외부 API 의존 소스 헬스체크 확장(UNIPASS·기상·환율) | 각 연동 API 자체 (신규 데이터 불필요) | 하 | P1 |
...
| **AISStream.io 웹소켓** | 무료 (키 즉시 발급) | 초 단위 스트리밍 | 지상파 AIS — 해안선 ~200km 이내(한국 연안 커뮤니티 수신국 존재, 공식 보장 없음). 원양 불가 | 중 — 서버측 상주 WebSocket(연결 후 3초 내 구독 JSON, BoundingBox·MMSI 필터), 재접속 처리. **브라우저 직결 불가(CORS)** | 베타·SLA 없음, 상용 이용 조건 약관 확인 필요 |
...
1. **1단계 (무료 PoC)** — AISStream.io를 서버측 백그라운드 워커에서 한국 해역 바운딩박스 + 관심 MMSI 필터로 구독 → Supabase 적재 → 자체 지도 표시. 비용 0으로 연안 실시간 확보. 현행 VesselFinder iframe은 글로벌 조망용으로 병행 유지.
...
| 순서 | 액션 | 소요/방식 |
|---|---|---|
| A-1 | **data.go.kr 회원가입 + 활용신청** → `DATA_GO_KR_KEY` 확보. 대상: 15006353, 15055851, 15006354, 15059059, 15059131, 15057250, 15084033, 15095068, 15113461, 15114086, 15157706, 15000415, 15059468, 3068846 | 자동승인 — 신청 즉시 사용 가능 |
| A-2 | **UNI-PASS 회원가입 + OpenAPI 별도 신청** → `UNIPASS_API_KEY` 확보 (화물통관진행정보 15126268 등) | data.go.kr 자동승인과 다른 별도 절차 |
| A-3 | **AISStream.io 키 발급** (GitHub 로그인 → API Keys 즉시 무료 발급) + 상용 이용 약관 확인 | 즉시 |
...
> **구현 현황 (2026-07-31)**: data.go.kr 공용 프록시 Edge Function `datago` 배포(별칭 화이트리스트: portmis·aircargo, needKey 안내 방식) + vessel.html에 **PORT-MIS 입출항 실적 조회 섹션** 구현(한/영/중 번역 포함). `DATA_GO_KR_KEY`를 Supabase Secrets에 등록하는 즉시(A-1) 실조회가 켜진다. 화물통관(track)도 동일 구조로 `UNIPASS_API_KEY` 대기 중.
...
6. status: 헬스체크 확장(신규 데이터 불필요 — **키 없이도 착수 가능**) · 물동량 벤치마크 위젯
...
- **선박 위치 1단계 PoC**: AISStream 워커 + Supabase 적재 + 지도 (A-3 이후)
