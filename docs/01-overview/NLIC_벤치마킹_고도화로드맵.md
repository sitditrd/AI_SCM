...
|---|---|---|---|
| 환율 KPI 스트립(USD/CNY 등) — **보류(2026-08-03)**: 3068846은 LINK형이라 data.go.kr이 키를 주지 않음, 수출입은행 자체 포털 발급 필요(A-7) | 수출입은행 환율 (3068846) | 하 | P1 (보류) |
| 항만 물동량 월간 KPI(TEU) — **구현 완료(2026-08-03)**: 히어로 TEU 스트립(수입·수출·합계·기준월) | 수출입컨테이너처리실적 (15059131) | 하 | P1 → 완료 |
| 기상특보 티커(항만 인접 해역) — **구현 완료(2026-08-03)**: 전국(stnId=108) 최근 2건, 특보 없으면 자동 숨김 | 기상청 기상특보 (15000415) | 하 | P1 → 완료 |

### insight (항만 혼잡 인사이트)
| 후보 기능 | 필요 API | 난이도 | 우선순위 |
|---|---|---|---|
| 포트 상세 월별 TEU 추이 오버레이 → **SECTION 07 독립 섹션으로 구현 완료(2026-08-03)**: 12개월 수입/수출 누적 막대 + 최신월 지역별 순위. 원천이 항만 단위가 아닌 상대 지역/국가 집계라 개별 항만 귀속 불가 | 국가별(15057250)·수출입(15059131) 컨테이너 실적 | 하 | P1 → 완료 |
...
| 후보 기능 | 필요 API | 난이도 | 우선순위 |
|---|---|---|---|
| 선박 클릭 시 제원 팝업(톤수·선종 등) — **구현 완료(2026-08-03)**: SHIP SPEC 섹션, 선박명/호출부호 조회 + 행 클릭 시 상세 15항목 | 선박제원정보 (15055851) | 하 | P1 → 완료 |
...
| 후보 기능 | 필요 API | 난이도 | 우선순위 |
|---|---|---|---|
| 항로별 컨테이너 물동량 근거 제시 — **구현 완료(2026-08-03)**: 시뮬레이션 KPI에 '한국 ↔ 도착국가' 실적 카드 추가(도착항 ISO2 매칭) | 국가별컨테이너처리실적 (15057250) | 하 | P1 → 완료 |
...
| 후보 기능 | 필요 API | 난이도 | 우선순위 |
|---|---|---|---|
| 항공 화물편 스케줄 탭(해상+항공 허브화) — **구현 완료(2026-08-03)**: AIR CARGO 섹션, 도착/출발 운항현황(15113461)·정기 운항편(15114086) 3개 탭 | 인천공항 화물기 정기운항편 (15114086) · 운항현황 상세 (15113461) | 하 | P1 → 완료 |
...
> **P1 화면 개발 완료(2026-08-03)** — `DATA_GO_KR_KEY` 등록·검증 후 아래 1~5를 모두 구현했다. 프록시 별칭 15종이 열려 있어 Edge Function 수정 없이 화면 코드만 추가했다.

1. ~~index: 물동량 TEU KPI · 기상특보 티커~~ **완료** — `teuimpexp` 최근 6개월 중 실적이 있는 최신월 합산 스트립 + `wthrwarn` 특보 티커(특보 없으면 자동 숨김). **환율 KPI는 보류**(A-7 수출입은행 자체 포털 키 필요)
2. ~~vessel: 선박 제원 팝업~~ **완료** — `shipspec` 선박명/호출부호 조회 + 행 클릭 시 상세 15항목 토글
3. ~~insight: 월별 TEU 추이~~ **완료** — SECTION 07 신설(앵커탭 Volume). 12개월 수입/수출 누적 막대 + 최신월 지역별 순위. 원천이 항만 단위가 아니라 **상대 지역(12개)/국가 집계**라 개별 항만 오버레이는 불가 → 독립 섹션으로 구현
4. ~~route: 항로별 물동량 근거~~ **완료** — 시뮬레이션 결과에 도착국 ISO2 코드로 매칭한 "한국 ↔ 도착국가" 실적 KPI 추가
5. ~~schedule: 항공 화물편 탭~~ **완료** — AIR CARGO 섹션 신설, 도착/출발 운항현황(15113461)·정기 운항편(15114086) 3개 탭

**구현 중 확인한 API 함정 4가지** (재작업 방지용 — 상세는 API.md §2.3)
- `teuimpexp`/`teunation` 필드 접두사가 직관과 **반대**: `e*` = 수입, `t*` = 수출
- 통계 3종 기간 파라미터는 **소문자 `sym`/`eym`**(대문자·`sYm` 아님) → 틀리면 코드 11
- `teuimpexp`·`teunation`은 `numOfRows` **상한 50** → 12개월(144행)·1개월 국가별(168행)은 페이지 순회 필수
- 해수부 원천은 **약 2개월 지연 공표** → 최신월 고정 조회 금지, 실적이 있는 월을 탐색해야 함
- 인천공항 운항현황(15113461) 시각은 **12자리 YYYYMMDDHHmm**, 정기편(15114086) `st`는 4자리 HHmm
