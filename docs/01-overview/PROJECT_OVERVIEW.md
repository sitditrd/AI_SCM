...
- **데이터 현황** (status.html, 관리자 전용) — 종합 판정 배너, 파이프라인 흐름도, 소스 신선도 게이지, 최근 7일 적재 타임라인, 이력, 외부 연동 헬스체크(Edge Function track/datago/send-code·Open-Meteo, 45초 주기 — needKey 응답은 "정상(키 대기)").
