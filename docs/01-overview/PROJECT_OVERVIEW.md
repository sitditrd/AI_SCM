...
- **경로 분석** (route.html) — 사전계산 항로 93건 기반 몬테카를로 소요일 분포, Voyager 지도 항로 시각화, 도착국가 컨테이너 물동량 근거 KPI(해수부 국가별 실적).
- **해외 스케줄** (schedule.html) — 선박 스케줄 3뷰는 준비중(기획 화면), **항공 화물편 스케줄은 실데이터 가동**(인천공항 도착/출발 운항현황·정기 운항편 3탭).
