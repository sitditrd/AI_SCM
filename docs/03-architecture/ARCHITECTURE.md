...
| 선사 API 키 | **등록 완료**: `HLAG_CLIENT_ID` · `HLAG_CLIENT_SECRET`(2026-08-21 — 앱 생성만으로는 부족하고 **Product Subscription** 까지 붙어야 403 이 풀린다. 엔드포인트는 `/hlag/external/v2/events`) · `ZIM_API_KEY` · `ZIM_CLIENT_ID` · `ZIM_CLIENT_SECRET`(2026-08-18 — ZIM 은 구독 키와 OAuth 토큰의 2단 관문이라 셋 다 있어야 통과한다). **미등록(어댑터는 배포 완료·키만 대기)**: `HMM_API_KEY` · `MAERSK_CONSUMER_KEY`(+`MAERSK_CLIENT_ID`/`MAERSK_CLIENT_SECRET`) · `CMACGM_API_KEY`(CMA 는 유료 확인으로 신청 보류). 키는 **Supabase Secrets 에만** 두고 저장소에 커밋하지 않으며, 요금 통지가 오면 **키만 삭제하면 `ready()` 가 false 가 되어 즉시 딥링크로 롤백**된다 |
