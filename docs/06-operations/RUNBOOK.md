...
### 3-b2. 적재는 됐는데 건수가 평소보다 적음 (PARTIAL)

증상: 데이터 현황 화면의 적재 이력에 **주황색 PARTIAL** 배지가 뜨고 건수가 평소(500 내외)보다
적다. 비고에 `미수집 N곳: BCT,BPT,...` 이 표시된다.

의미: **적재는 정상이고, 원본 엑셀에 해당 터미널 시트가 아예 없었다**는 뜻이다. 즉 06시
수집기가 그 터미널을 못 긁은 것이므로 적재기를 손댈 일이 아니다.

실측 사례(2026-08-12·08-13): BPT(115건)·BCT·DDCT·PNCT 누락으로 510건 → 363건.
이때는 status 가 `SUCCESS` 로 하드코딩되어 있어 **초록색으로 정상 표시**됐다(2026-08-14 수정).

조치:

1. 원본 확인 — `D:\터미널 스케쥴 정보\통합\YYYY\MM\` 아래 해당일 통합 파일의 시트 목록에
   그 터미널이 있는지 본다. 없으면 수집기 문제다.
2. 수집기 로그에서 그 터미널의 실패 사유를 확인한다(사이트 개편·타임아웃·차단).
3. 원본을 다시 받았다면 `python scripts\upload_berth_sql_parts.py --rest --today` 로 재적재한다
   (동일 수집일 replace 라 안전).

> 상태 판정은 `collect_upload_berth.log_verdict()` 한 곳에 있다. 세 적재기(일일·스케줄·백필)가
> 모두 이 함수를 쓰므로 규칙을 바꾸려면 여기만 고치면 된다.
> **주의**: 이 함수는 적재가 실제로 일어난 뒤에만 호출한다. `per_terminal` 이 비면 `UNKNOWN` 을
> 돌려주는데, 이는 '문제 없음'이 아니라 '판단 근거 없음'이다 — 과거 로그를 일괄 재판정하다
> '수집 파일 없음' FAIL 행이 SUCCESS 로 뒤집힌 적이 있어(2026-08-14, 즉시 복구) 넣은 가드다.
