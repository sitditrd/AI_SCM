...
### 3-b2. 적재는 성공인데 건수가 평소보다 적음 (터미널 누락)

증상: 적재 이력의 상태는 **SUCCESS** 인데 건수가 평소(500 내외)보다 적고, 비고에
**`미수집 N곳: BCT,BPT,...`** 이 주황색으로 표시된다.

의미: **적재는 정상이다.** 원본 통합 엑셀에 그 터미널 시트가 아예 없었다는 뜻이므로
06시 수집 쪽 문제이고, 적재기를 손댈 일이 아니다.

실측(2026-08-12·08-13): BPT(약 115건)·BCT·DDCT·PNCT 누락으로 510건 → 363건.

조치:

1. 원본 확인 — `D:\터미널 스케쥴 정보\통합\YYYY\MM\` 아래 해당일 통합 파일의 시트 목록에
   그 터미널이 있는지 본다. 없으면 수집기 문제다.
2. 수집기 로그에서 실패 사유를 확인한다(사이트 개편·타임아웃·차단).
3. 원본을 다시 받았다면 `python scripts\upload_berth_sql_parts.py --rest --today` 로 재적재
   (동일 수집일 replace 라 안전).

> **상태와 데이터 완전성은 분리한다.** 상태(SUCCESS/FAIL)는 '적재 작업이 성공했는가' 만
> 나타내고, 어느 터미널이 빠졌는지는 비고에만 적는다. 한때 누락이 있으면 상태를 PARTIAL 로
> 낮췄으나 적재에 성공한 건이 화면에서 실패처럼 읽혀 되돌렸다(2026-08-14).
> 판정은 `collect_upload_berth.log_verdict()` 한 곳에 있고 세 적재기가 공유한다.
> `per_terminal` 이 비면 `UNKNOWN` 을 돌려주는데, 이는 '문제 없음'이 아니라 '판단 근거 없음'
> 이다 — 과거 로그 일괄 재판정 중 '수집 파일 없음' FAIL 행이 SUCCESS 로 뒤집힌 적이 있어
> (2026-08-14, 즉시 복구) 넣은 가드다.
