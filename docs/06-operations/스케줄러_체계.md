...
```bat
REM 1) 키 등록 (관리자 권한 콘솔) — Supabase 대시보드 → Settings → API → service_role
setx SUPABASE_SERVICE_KEY "<service_role key>"
REM 2) 작업 등록 (06시 수집 직후)
schtasks /create /tn "TWL_BerthUpload" /tr "\"<REPO>\scripts\run_berth_upload.bat\"" /sc daily /st 06:40 /f
```

- 실행 로그: `logs/berth_upload.log` (키 미설정이면 `[SKIP]` 만 남기고 조용히 종료)
- **트레이드오프**: 이 경로는 service_role 키를 PC에 저장해야 한다. 키 미보관 원칙을 유지하려면 등록하지 말고 앱 캐치업에 의존한다(데이터는 결국 적재되며 표시만 지연됨).
